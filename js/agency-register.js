// 파일 업로드 관련 변수
let agencyBusinessLicenseFile = null;

// 파일 업로드 기능
function initAgencyFileUpload() {
  const uploadArea = document.getElementById("agency-business-license-upload");
  const fileInput = document.getElementById("agency-business-license");
  const fileInfo = document.getElementById("agency-business-license-info");
  const fileName = document.getElementById("agency-business-license-name");
  const fileSize = document.getElementById("agency-business-license-size");
  const removeBtn = document.getElementById("remove-agency-business-license");
  ...
  removeBtn.addEventListener("click", () => {
    agencyBusinessLicenseFile = null;
    fileInput.value = "";
  ...
  function handleFileSelect(file) {
    ...
    agencyBusinessLicenseFile = file;
...
// 페이지 로드 시 파일 업로드 초기화
document.addEventListener("DOMContentLoaded", initAgencyFileUpload);
...
document
  ...
  .addEventListener("submit", async function (e) {
    ...
    // 사업자등록증 파일 검증
    if (!agencyBusinessLicenseFile) {
      alert("사업자등록증을 업로드해주세요.");
      return;
    }

    const userData = {
      username: document.getElementById("agencyId").value,
      password: pw,
      agency_name: document.getElementById("agencyName").value,
      business_number: document.getElementById("businessNumber").value,
      manager_name: document.getElementById("agencyContact").value,
      phone: document.getElementById("agencyPhone").value,
      email: document.getElementById("agencyEmail").value,
      website_url: document.getElementById("agencyWebsite").value,
      business_license: agencyBusinessLicenseFile, // 파일 객체 추가
    };

    const result = await window.authAPI.signupAgency(userData);
    if (result.success) {
      // 가입 성공 후 사업자등록증 파일을 사용자별 IndexedDB에 저장
      try {
        const userId = result?.data?.user?.id || "";
        if (userId && agencyBusinessLicenseFile) {
          await saveBusinessLicenseToIDB(
            userId,
            agencyBusinessLicenseFile,
            "agency"
          );
          const meta = {
            fileName: agencyBusinessLicenseFile.name,
            fileSize: agencyBusinessLicenseFile.size,
            fileType: agencyBusinessLicenseFile.type,
      ...
      try {
        ...
        if (userId) {
          ...
          localStorage.setItem("troy_user_id_agency", userId);
          // 레거시 호환을 위해 troy_user_id도 저장 (현재 역할용)
          localStorage.setItem("troy_user_id", userId);

          // 사용자별 프로필 저장
          const profile = {
            companyName: document.getElementById("agencyName").value || "",
            businessNumber:
              document.getElementById("businessNumber").value || "",
            managerName: document.getElementById("agencyContact").value || "",
            phone: document.getElementById("agencyPhone").value || "",
            email: document.getElementById("agencyEmail").value || "",
          };
          localStorage.setItem(
          ...
          if (!localStorage.getItem(mcKey)) {
            ...
            localStorage.setItem(mcKey, `AG${idPart}`);
          }
        }
      } catch (_) {}
      ...
      const userInfo = {
        ...
        memberCode: "AG" + Date.now().toString().slice(-6),
      };
      // 역할별 userInfo 저장 (다른 역할과 분리)
      localStorage.setItem("userInfo_agency", JSON.stringify(userInfo));
