/**
 * 파일: js/client-management.js
 * 목적: 클라이언트 관리 및 대량 캠페인 등록 시스템의 핵심 로직
 *       클라이언트 CRUD, 캠페인 관리, 상품 스크래핑, 결제 처리 등을 담당
 */
...
class TroyBulkCampaign {
  constructor() {
    ...
    // 테스트용 클라이언트 추가 (개발 단계에서만)
    this.addTestClients();

    this.init();
  }

  // 테스트용 클라이언트 데이터 추가 (개발 단계용)
  addTestClients() {
    // 기존에 클라이언트가 없다면 테스트용 클라이언트 추가
    if (this.clients.length === 0) {
      const testClients = [
        ...
        },
        {
          id: "test-client-3",
          name: "글로벌 마케팅",
          businessName: "Global Marketing Inc.",
          url: "https://global-marketing.com",
          businessNumber: "555-88-99000",
          createdAt: new Date().toISOString(),
          totalCampaigns: 12,
          activeCampaigns: 4,
      ...
      // 테스트용 캠페인 데이터도 추가
      this.campaigns["test-client-1"] = [];
      this.campaigns["test-client-2"] = [];
      this.campaigns["test-client-3"] = [];
  ...
  renderClientDashboardGrid() {
    ...
    grid.innerHTML = this.clients
      .map((client, index) => {
        ...
        return `
                <div class="client-folder-card" data-client-id="${client.id}" style="
                    background: white;
                ...
                " onmouseover="this.style.transform='translateY(-2px)'; this.style.boxShadow='0 4px 12px rgba(0,0,0,0.15)'; this.style.borderColor='#255ffe';" 
                   onmouseout="this.style.transform='translateY(0)'; this.style.boxShadow='0 2px 8px rgba(0,0,0,0.1)'; this.style.borderColor='#e5e7eb';">
                    ...
                    <div style="text-align: center; margin-bottom: 16px;">
                        ...
                        <div style="font-size: 14px; color: #6b7280;">${client.businessName}</div>
                    </div>
                    ...
                    <div style="text-align: center; margin-bottom: 20px;">
                        ...
                        ">📁</div>
                        <div style="font-size: 24px; font-weight: 800; color: #255ffe; margin-bottom: 4px;">
                            ${client.progress}%
                        </div>
                        <div style="font-size: 12px; color: #6b7280;">진행률</div>
                    </div>
                    ...
                    <div style="
                        ...
                        gap: 8px;
                    ">
                        <button onclick="event.stopPropagation(); app.showClientDetail('${client.id}')" style="
                            flex: 1;
                            padding: 12px;
                            background: #255ffe;
                            color: white;
                            ...
                            transition: all 0.2s ease;
                        ">캠페인 보기</button>
                        <button onclick="event.stopPropagation(); app.showBulkCampaignModal()" style="
                            flex: 1;
                            padding: 12px;
                            background: #f3f4f6;
                            color: #374151;
    ...
    // Add click handlers
    grid.querySelectorAll(".client-folder-card").forEach((card) => {
...
// Initialize when DOM is ready or when loaded dynamically
