function saveTaxInfo() {
  ...
  // 실제로는 서버에 전송해야 함
  console.log("세금계산서 정보 저장:", formData);
  alert("세금계산서 발급정보가 저장되었습니다.");
}
...
async function loadCustomerUserInfo() {
  try {
    const role =
    ...
    // 고객사는 advertiser 또는 customer 역할일 수 있음
    const roleForToken = role === "advertiser" ? "customer" : role;
    const token =
      typeof getRoleSessionToken === "function"
        ? getRoleSessionToken(role)
        : sessionStorage.getItem(`troy_token_${roleForToken}`) ||
    ...
    const userId = (
      ...
      localStorage.getItem(`troy_user_id_${roleForToken}`) ||
    ...
    try {
      ...
      const roleForInfo = role === "advertiser" ? "customer" : role;
      const legacy =
        localStorage.getItem(`userInfo_${roleForInfo}`) ||
      ...
      const parsed = rawLocal
        ...
        : null;
      if (parsed) {
        companyName = parsed.companyName || parsed.company_name || companyName;
        businessNumber =
    ...
    // 데이터가 없을 경우 안내 문구
    const noDataMessage = "입력된 정보가 없습니다";

    // DOM 반영
    const companyNameInput = document.getElementById("customer-company-name");
    if (companyNameInput) {
      companyNameInput.value = companyName || noDataMessage;
      if (!companyName) {
        companyNameInput.style.color = "#64748b";
        companyNameInput.style.fontStyle = "italic";
      }
    }
    ...
    if (businessNumberInput) {
      businessNumberInput.value = businessNumber || noDataMessage;
      if (!businessNumber) {
        businessNumberInput.style.color = "#64748b";
        businessNumberInput.style.fontStyle = "italic";
      }
    }

    const memberCodeElements = document.querySelectorAll(".info-value");
    if (memberCodeElements.length > 0) {
      memberCodeElements[0].textContent = memberCode || noDataMessage;
      if (!memberCode) {
        memberCodeElements[0].style.color = "#64748b";
        memberCodeElements[0].style.fontStyle = "italic";
      }
    }
    ...
    if (joinDateElement) {
      ...
      if (savedJoin) {
        joinDateElement.textContent = savedJoin;
      } else {
        joinDateElement.textContent = noDataMessage;
        joinDateElement.style.color = "#64748b";
        joinDateElement.style.fontStyle = "italic";
      }
    }
    ...
    if (managerNameInput) {
      managerNameInput.value = managerName || noDataMessage;
      if (!managerName) {
        managerNameInput.style.color = "#64748b";
        managerNameInput.style.fontStyle = "italic";
      }
    }
    ...
    if (emailInput) {
      emailInput.value = email || noDataMessage;
      if (!email) {
    ...
    if (phoneInput) {
      phoneInput.value = phone || noDataMessage;
      if (!phone) {
  ...
  try {
    const role =
      typeof getCurrentUserRole === "function"
        ...
        : "advertiser";
    // 고객사는 advertiser 또는 customer 역할일 수 있음
    const roleForUserId = role === "advertiser" ? "customer" : role;
    // 역할별 userId 우선 조회 (현재 역할의 userId만 사용)
    const userId = (
      localStorage.getItem(`troy_user_id_${role}`) ||
      localStorage.getItem(`troy_user_id_${roleForUserId}`) ||
      localStorage.getItem("troy_user_id") ||
      ""
    ).trim();
    if (!userId) {
    ...
    window.__currentBusinessLicenseRole = roleForUserId;
...
function initCustomerMyInfo() {
  // 사용자 정보 로드
  loadCustomerUserInfo();
  // 체크박스 토글 기능
  const checkbox = document.getElementById("doNotDisturb");
  if (checkbox) {
    checkbox.addEventListener("change", function () {
      if (this.checked) {
        console.log("방해금지모드 활성화");
      } else {
        console.log("방해금지모드 비활성화");
      }
    });
  }

  // 폼 입력 필드 포커스 효과
  const inputs = document.querySelectorAll(".form-input, .tax-input");
    ...
    saveBtn.addEventListener("click", function () {
      const formData = {
        ...
        email: document.querySelector(".contact-form input[type='email']")
          .value,
        phone: document.querySelector(".contact-form input[type='tel']").value,
        doNotDisturb: document.getElementById("doNotDisturb").checked,
      };

      // 실제로는 서버에 전송해야 함
      console.log("담당자 정보 저장:", formData);
      alert("담당자 정보가 저장되었습니다.");
    });
  }

  // 변경하기 버튼 클릭 이벤트
  const changeBtns = document.querySelectorAll(".btn-secondary");
  changeBtns.forEach((btn) => {
    btn.addEventListener("click", function () {
      const label = this.parentElement.querySelector(".info-label").textContent;
      alert(`${label} 변경 기능은 준비 중입니다.`);
    });
  });
}

// 전역 노출
window.saveTaxInfo = saveTaxInfo;
window.closeUserInfoModal = closeUserInfoModal;
window.initCustomerMyInfo = initCustomerMyInfo;

// =========================
// IndexedDB 접근: 고객사 등록증 조회용
// =========================
