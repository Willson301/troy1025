// 파트너사 스케줄 관리
let partnerSchedulesCurrentDate = new Date();
...
// 실제 캠페인에서 스케줄 생성
async function loadPartnerSchedulesFromAPI() {
  ...
  try {
    ...
    return items.map((c, idx) => {
      const s = new Date(c.start_date || c.created_at || now);
      const e = new Date(c.end_date || c.created_at || now);
      return {
        id: c.id || idx + 1,
        campaignName:
          c.title || c.product_title || c.campaign_name || `캠페인 ${idx + 1}`,
        clientName:
          c.brand_name || c.client_name || c.company_name || "클라이언트",
        startDate: `${s.getFullYear()}-${String(s.getMonth() + 1).padStart(
          2,
          "0"
        )}-${String(s.getDate()).padStart(2, "0")}T09:00`,
        endDate: `${e.getFullYear()}-${String(e.getMonth() + 1).padStart(
          2,
          "0"
        )}-${String(e.getDate()).padStart(2, "0")}T18:00`,
        status: "진행중",
        priority: "보통",
        progress: 0,
        assignee: "",
        type: c.campaign_type || "구매평",
      };
    });
  } catch (e) {
    console.error("loadPartnerSchedulesFromAPI error", e);
    return [];
  }
}

// 기존 함수 재정의: generateCurrentMonthSchedules -> API 기반
async function generateCurrentMonthSchedules() {
  const api = await loadPartnerSchedulesFromAPI();
  if (api && api.length) return api;
  return [];
}

let partnerSchedules = [];

async function initPartnerScheduleManagement() {
  // 데이터 로드
  partnerSchedules = await generateCurrentMonthSchedules();
...
function updateCalendarHeader() {
  ...
  const year = partnerSchedulesCurrentDate.getFullYear();
  const month = monthNames[partnerSchedulesCurrentDate.getMonth()];
...
function renderCalendarView() {
  ...
  // 클라이언트별 색상 범례 생성
  const uniqueClients = [...new Set(partnerSchedules.map((s) => s.clientName))];
...
function generateCalendarGrid() {
  ...
  const first = new Date(
    partnerSchedulesCurrentDate.getFullYear(),
    partnerSchedulesCurrentDate.getMonth(),
    1
  );
  const last = new Date(
    partnerSchedulesCurrentDate.getFullYear(),
    partnerSchedulesCurrentDate.getMonth() + 1,
    0
  );
  ...
  for (let i = 0; i < total; i++) {
    ...
    if (date.getMonth() !== partnerSchedulesCurrentDate.getMonth()) {
...
function getSchedulesForDate(date) {
  return partnerSchedules.filter((s) => {
...
function renderTimelineView() {
  const container = document.getElementById("schedule-content");
  const sortedSchedules = [...partnerSchedules].sort(
...
function renderGanttView() {
  ...
  // 현재 월에 해당하는 일정만 필터링
  const currentYear = partnerSchedulesCurrentDate.getFullYear();
  const currentMonth = partnerSchedulesCurrentDate.getMonth();

  // 간트차트에서는 모든 일정을 표시 (월 필터링 제거)
  const filteredSchedules = partnerSchedules;
  ...
  const startDate = new Date(
    Math.min(...partnerSchedules.map((s) => new Date(s.startDate)))
  );
  const endDate = new Date(
    Math.max(...partnerSchedules.map((s) => new Date(s.endDate)))
  );
  const totalDays =
    Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
...
function getClientColor(clientName) {
  const colors = [
    "#38bdf8", // 적당한 하늘색 - 백마로지스
    "#4ade80", // 적당한 연두색 - 바눈
    "#2dd4bf", // 적당한 민트 - 킹기문주식회사
    "#60a5fa", // 적당한 블루 - 스마트기기
    "#8b5cf6", // 적당한 퍼플 - 패션브랜드
    "#22d3ee", // 적당한 시안 - 뷰티제품
    "#84cc16", // 적당한 라임 - 식품업체
...
function changeMonth(dir) {
  partnerSchedulesCurrentDate.setMonth(
    partnerSchedulesCurrentDate.getMonth() + dir
  );
  updateCalendarHeader();
...
function viewSchedule(id) {
  const s = partnerSchedules.find((x) => x.id === id);
  if (!s) return;
...
// 전역 함수 등록
window.initPartnerScheduleManagement = initPartnerScheduleManagement;
