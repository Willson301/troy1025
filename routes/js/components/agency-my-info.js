...
async function loadAgencyUserInfo() {
  try {
    const role =
    ...
    if (token) {
      try {
        ...
        if (res.ok) {
          ...
          const u = data?.user || {};
          // 대행사 역할 필드명: agency_name 우선, company_name 보조
          companyName = u.agency_name || u.company_name || companyName;
    ...
    // 사용자별 로컬 프로필 보조 (가입 시 저장값 등) - 대행사 역할에 맞게
    try {
      ...
      if (parsed) {
        // 대행사 역할 필드명 우선 조회
        companyName = parsed.companyName || parsed.agency_name || companyName;
        businessNumber =
      ...
      if (!rawLocal && legacy && userId) {
        ...
        // 대행사 역할인 경우만 저장
        if (legacyParsed.userType === "agency") {
    ...
    // DOM 반영
    const companyNameInput = document.getElementById("agency-company-name");
    if (companyNameInput) {
...
function initAgencyMyInfo() {
  ...
  // 폼 입력 필드 이벤트 리스너
  const formInputs = document.querySelectorAll(".form-input, .tax-input");
  formInputs.forEach((input) => {
    input.addEventListener("focus", function () {
      this.style.borderColor = "#255ffe";
      this.style.boxShadow = "0 0 0 3px rgba(37, 95, 254, 0.1)";
    });
...
window.initAgencyMyInfo = initAgencyMyInfo;

// =========================
// IndexedDB 접근: 대행사 등록증 조회용
// =========================
