...
// Supabase에서 고객 결제 데이터 로드
async function loadCustomerPaymentDataFromSupabase(page = 1, filters = {}) {
  try {
    if (!supabase) {
    ...
    paymentData = (data || []).map((payment, index) => {
      ...
      return {
        ...
        clientContact: campaign?.requirements?.contact_name || "-",
        ...
        tags: campaign?.requirements?.tags || [],
      };
    });

    console.log(
      `Supabase에서 고객 결제 데이터 로드 완료: 페이지 ${page}/${totalPages}, 총 ${totalCount}개 중 ${paymentData.length}개`
    );

    // 통계 업데이트
    await updateCustomerPaymentStatistics();
  } catch (error) {
    console.error("고객 결제 데이터 로드 실패:", error);
    paymentData = [];
    totalCount = 0;
    totalPages = 1;
    currentPage = 1;
...
// 고객 결제 통계 업데이트
async function updateCustomerPaymentStatistics() {
  try {
    if (!supabase) {
      console.log(
    ...
    console.error("고객 결제 통계 업데이트 실패:", error);
  }
}
...
async function applyFilters() {
  ...
  currentPage = 1;
  await loadCustomerPaymentDataFromSupabase(1, filters);
...
async function resetFilters() {
  ...
  currentPage = 1;
  await loadCustomerPaymentDataFromSupabase(1, {});
  updatePaymentTable(paymentData);
  updatePagination();
}
...
async function goToPage(page) {
  ...
  await loadCustomerPaymentDataFromSupabase(page, filters);
  updatePaymentTable(paymentData);
  updatePagination();
...
async function initCustomerPaymentHistory() {
  try {
    // Supabase에서 고객 결제 데이터 로드 (첫 페이지)
    await loadCustomerPaymentDataFromSupabase(1, {});
    ...
    console.log("고객 결제내역 컴포넌트 초기화 완료");
  } catch (error) {
    console.error("고객 결제내역 컴포넌트 초기화 실패:", error);
  }
}
...
window.initCustomerPaymentHistory = initCustomerPaymentHistory;
