// 대행사 스케줄 관리
let agencySchedulesCurrentDate = new Date();
...
function initAgencyScheduleManagement() {
  bindViewButtons();
...
function updateCalendarHeader() {
  ...
  const year = agencySchedulesCurrentDate.getFullYear();
  const month = monthNames[agencySchedulesCurrentDate.getMonth()];
...
function renderCalendarView() {
  ...
  // 클라이언트별 색상 범례 생성
  const uniqueClients = [...new Set(agencySchedules.map((s) => s.clientName))];
  const legend = uniqueClients
    .map(
      (client) => `
    <div style="display: flex; align-items: center; gap: 8px;">
      <div style="width: 16px; height: 16px; background: ${getClientColor(
        client
      )}; border-radius: 4px;"></div>
      <span style="font-size: 12px; color: #374151;">${client}</span>
    </div>
  `
    )
    .join("");
  ...
  container.innerHTML = `
    <div style="margin-bottom: 16px; padding: 12px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0;">
      <div style="font-size: 14px; font-weight: 600; color: #374151; margin-bottom: 8px;">클라이언트별 색상 구분</div>
      <div style="display: flex; flex-wrap: wrap; gap: 16px;">
        ${legend}
      </div>
    </div>
    <div id="calendarGrid" style="display: grid; grid-template-columns: repeat(7, minmax(120px, 1fr)); gap: 1px; background: #e2e8f0; border-radius: 8px; overflow-x: auto; overflow-y: hidden;"></div>
  `;
...
function generateCalendarGrid() {
  ...
  ["일", "월", "화", "수", "목", "금", "토"].forEach((d) => {
    ...
    hd.textContent = d;
    hd.style.cssText =
      "background:linear-gradient(135deg,#f8fafc 0%,#e2e8f0 100%);padding:16px 8px;text-align:center;font-weight:600;color:#64748b;border-bottom:2px solid #e2e8f0;position:sticky;top:0;z-index:1;";
    grid.appendChild(hd);
  });

  const first = new Date(
    agencySchedulesCurrentDate.getFullYear(),
    agencySchedulesCurrentDate.getMonth(),
    1
  );
  const last = new Date(
    agencySchedulesCurrentDate.getFullYear(),
    agencySchedulesCurrentDate.getMonth() + 1,
    0
  );
  ...
  for (let i = 0; i < total; i++) {
    ...
    cell.style.cssText =
      "background: white; min-height: 120px; padding: 8px; border: 1px solid #e2e8f0; position: relative; min-width:120px;";

    if (date.getMonth() !== agencySchedulesCurrentDate.getMonth()) {
    ...
    const max = 3; // 한 날짜 셀에 최대 3개까지 직접 표시
    ds.slice(0, max).forEach((s) => {
      const ev = document.createElement("div");
      ev.textContent = truncateText(s.campaignName, 40);
      ev.title = `${s.campaignName}\n${s.clientName}\n상태: ${s.status}`;
      ev.style.cssText = `background: ${getClientColor(
        s.clientName
    ...
    // '+a' 표시는 5개 초과(=max 초과)일 때만 노출
    if (ds.length > max) {
      const more = document.createElement("div");
      const hiddenCount = ds.length - max;
      more.textContent = `+${hiddenCount} 더보기`;
      more.title = `추가 일정 ${hiddenCount}개`;
      more.style.cssText =
...
function getSchedulesForDate(date) {
  return agencySchedules.filter((s) => {
      ...
      <table style="width: 100%; border-collapse: collapse;">
        <thead style="background: #f8fafc;">
          <tr>
            ...
            <th style="padding: 12px; text-align: left; border-bottom: 1px solid #e2e8f0; font-weight: 600;">클라이언트</th>
        ...
        <tbody>
          ${agencySchedules
            ...
            <tr style="border-bottom: 1px solid #f1f5f9; cursor: pointer;" onclick="viewSchedule(${
              ...
              <td style="padding: 12px;">${schedule.clientName}</td>
...
function renderTimelineView() {
  const container = document.getElementById("schedule-content");
  const sortedSchedules = [...agencySchedules].sort(
          ...
          .map((schedule, index) => {
            ...
            return `
              <div style="display: flex; align-items: center; margin-bottom: 24px; position: relative;">
                <div style="width: 12px; height: 12px; background: ${getClientColor(
                  schedule.clientName
...
function renderGanttView() {
  ...
  // 현재 월에 해당하는 일정만 필터링
  const currentYear = agencySchedulesCurrentDate.getFullYear();
  const currentMonth = agencySchedulesCurrentDate.getMonth();

  // 간트차트에서는 모든 일정을 표시 (월 필터링 제거)
  const filteredSchedules = agencySchedules;
  ...
  const startDate = new Date(
    Math.min(...agencySchedules.map((s) => new Date(s.startDate)))
  );
  const endDate = new Date(
    Math.max(...agencySchedules.map((s) => new Date(s.endDate)))
  );
  const totalDays =
    Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
          ...
          <div style="display: grid; grid-template-columns: 300px 1fr; border-bottom: 1px solid #f1f5f9; position: relative;">
            ...
            <div style="display: grid; grid-template-columns: repeat(${totalDays}, 1fr); position: relative; min-height: 44px;">
              ...
              <div style="
                ...
                height: 20px;
                background: ${getClientColor(schedule.clientName)};
                border-radius: 4px;
                cursor: pointer;
                display: flex;
                align-items: center;
...
function getClientColor(clientName) {
  const colors = [
    "#38bdf8", // 적당한 하늘색 - ABC 쇼핑몰
    "#4ade80", // 적당한 연두색 - 뷰티 브랜드
    "#2dd4bf", // 적당한 민트 - 홈데코 브랜드
    "#60a5fa", // 적당한 블루 - 스마트기기
    "#8b5cf6", // 적당한 퍼플 - 패션브랜드
    "#22d3ee", // 적당한 시안 - 뷰티제품
    "#84cc16", // 적당한 라임 - 식품업체
...
function changeMonth(dir) {
  agencySchedulesCurrentDate.setMonth(
    agencySchedulesCurrentDate.getMonth() + dir
  );
  updateCalendarHeader();
...
function viewSchedule(id) {
  const s = agencySchedules.find((x) => x.id === id);
  if (!s) return;
  ...
  modal.innerHTML = `
    <div style="background: white; border-radius: 8px; padding: 24px; max-width: 500px; width: 90%; position: relative;">
      ...
      <div style="margin-bottom: 16px;">
        <strong>클라이언트:</strong> ${s.clientName}
      </div>
      <div style="margin-bottom: 16px;">
...
function showAllSchedules(date, list) {
  ...
  // 기존 모달 제거
  const existing = document.getElementById("allSchedulesModal");
  ...
  const itemsHtml = (list || [])
    .map(
      (s, idx) => `
      <div style="border: 1px solid #e5e7eb; border-radius: 12px; padding: 12px; margin-bottom: 12px; display: flex; justify-content: space-between; align-items: center; gap: 12px;">
        <div style="min-width: 0;">
          <div style="font-weight: 700; color: #111827; margin-bottom: 4px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${
            idx + 1
          }. ${esc(s.campaignName)}</div>
          <div style="font-size: 12px; color: #6b7280;">
            ${esc(s.clientName || "클라이언트 미상")}
            <span style="margin: 0 6px; color: #d1d5db;">•</span>
            ${fmt(s.startDate)} ~ ${fmt(s.endDate)}
          </div>
        </div>
        <button onclick="viewSchedule('${esc(
          s.id
        )}')" style="padding: 8px 12px; background: #f9fafb; color: #374151; border: 1px solid #d1d5db; border-radius: 8px; cursor: pointer; font-weight: 600; white-space: nowrap;">상세</button>
      </div>`
    )
    .join("");

  modal.innerHTML = `
    <div class="modal-content" style="background-color: white; margin: 5% auto; padding: 24px; border-radius: 16px; width: 95%; max-width: 640px; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 60px rgba(0,0,0,0.3);">
      <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #e5e7eb; padding-bottom: 12px;">
        <h2 style="margin: 0; color: #1f2937; font-size: 20px; font-weight: 700;">${esc(
          dateLabel
        )} 전체 일정 <span style="color:#6b7280; font-size:14px;">(${
    list?.length || 0
  })</span></h2>
        <span class="close" onclick="closeAllSchedulesModal()" style="font-size: 28px; cursor: pointer; color: #6b7280; font-weight: bold;">&times;</span>
      </div>
      <div class="modal-body" style="padding-top: 4px;">
        ${
          itemsHtml ||
          '<div style="color:#6b7280; font-size:14px;">표시할 일정이 없습니다.</div>'
        }
      </div>
    </div>
  `;
  ...
  // 외부 클릭 닫기
  modal.addEventListener("click", (e) => {
    if (e.target === modal) closeAllSchedulesModal();
  });
}
...
// 전역 함수 등록
window.initAgencyScheduleManagement = initAgencyScheduleManagement;
window.switchView = switchView;
window.changeMonth = changeMonth;
window.viewSchedule = viewSchedule;
window.closeScheduleDetailModal = closeScheduleDetailModal;
window.showAllSchedules = showAllSchedules;
window.closeAllSchedulesModal = closeAllSchedulesModal;
